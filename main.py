...
# -------------------------
LENYA_POR_GALLINA = 6        # 6 kg leña = 1 gallina
LENYA_POR_PATATA_KG = 4 / 3  # 2 kg leña = 1.5 kg patatas
LENYA_POR_CABRA = 5          # 5 kg leña = 1 cabra
LENYA_POR_HUEVO = 0.25       # 3 kg leña = 12 huevos
LENYA_POR_CABALLO = 12       # 12 kg leña = 1 caballo
...
# -----------------------------
def validar_positivo(cant: number) -> bool:
    # Devuelve True si la cantidad es mayor que 0.
    return cant > 0

def validar_entero(cant: number) -> bool:
    # Devuelve True si la cantidad no tiene decimales.
    return cant % 1 == 0

def mostrar_error(msg: str):
    # Muestra mensajes de error para utilizarlos cuando se necesiten.
    game.show_long_text(msg, DialogLayout.CENTER)

def reactivar_menu():
    # Permite abrir el menú de nuevo después de hacer un trueque.
    global menu_activo
    menu_activo = False
    dibujar_menu()
...
# ---------------------
def calcular_lenya_gallinas(num: number):
    return Math.round(num * LENYA_POR_GALLINA * 100) / 100

def calcular_lenya_huevos(num: number):
    return Math.round(num * LENYA_POR_HUEVO * 100) / 100

def calcular_lenya_cabras(num: number):
    return Math.round(num * LENYA_POR_CABRA * 100) / 100

def calcular_lenya_patatas(kg: number):
    return Math.round(kg * LENYA_POR_PATATA_KG * 100) / 100

def calcular_lenya_caballos(num: number):
    return Math.round(num * LENYA_POR_CABALLO * 100) / 100
...
def trueque_gallina():
    """
    Gestiona el trueque de gallinas por leña.
    - Cierra el menú y activa el modo trueque, esto significa que bloquea nuevos trueques mientras se está haciendo.
    - Pide al usuario cuántas gallinas quiere.
    - Valida que la cantidad sea positiva y entera.
    - Calcula los kg de leña que son y los muestra.
    - Reactiva el menú al terminar.
    """
    global en_trueque, cantidad, kilos, mensaje
    en_trueque = True
    myMenu.close()
    cantidad = game.ask_for_number("Cuántas gallinas deseas?", 4)

    if not (validar_positivo(cantidad)):
        mostrar_error("Error: Debe ser mayor que 0")
        en_trueque = False
        reactivar_menu()
        return
    if not (validar_entero(cantidad)):
        mostrar_error("Error: Solo gallinas enteras!")
        en_trueque = False
        reactivar_menu()
        return

    kilos = calcular_lenya_gallinas(cantidad)
    mensaje = "" + str(int(cantidad)) + " gallinas son " + ("" + str(kilos)) + " kg de leña"
...
def trueque_cabras():
    """
    Gestiona el trueque de cabras por leña.
    - Cierra el menú y activa el modo trueque.
    - Pide el número de cabras.
    - Comprueba que sea mayor que 0 y sin decimales, sino lanza error.
    - Calcula la leña equivalente y muestra el mensaje final.
    - Finaliza el trueque y reabre el menú.
    """
    global en_trueque, cantidad, kilos, mensaje
    en_trueque = True
    myMenu.close()
    cantidad = game.ask_for_number("Cuántas cabras deseas?", 4)
    ...
    if not (validar_positivo(cantidad)):
        ...
        return
    if not (validar_entero(cantidad)):
        mostrar_error("Error: Solo cabras enteras!")
        en_trueque = False
        reactivar_menu()
        return

    kilos = calcular_lenya_cabras(cantidad)
    mensaje = "" + str(int(cantidad)) + " cabras son " + ("" + str(kilos)) + " kg de leña"
    game.show_long_text(mensaje, DialogLayout.CENTER)
    en_trueque = False
    reactivar_menu()
...
def trueque_patatas():
    """
    Gestiona el trueque de patatas (kg) por leña.
    - Cierra el menú y activa el modo trueque.
    - Solicita los kg de patatas deseados.
    - Valida que la cantidad sea positiva (se permite decimal en este).
    - Calcula los kg de leña para esos kg de patatas.
    - Muestra el resultado y reabre el menú.
    """
    global en_trueque, cantidad, kilos, mensaje
    en_trueque = True
    myMenu.close()
    cantidad = game.ask_for_number("Cuántos kg patatas deseas?", 4)
    ...
    kilos = calcular_lenya_patatas(cantidad)
    mensaje = "" + str(cantidad) + " kg patatas son " + ("" + str(kilos)) + " kg de leña"
    game.show_long_text(mensaje, DialogLayout.CENTER)
    en_trueque = False
    reactivar_menu()
    dibujar_menu()

def trueque_caballos():
    """
    Gestiona el trueque de caballos por leña.
    - Cierra el menú y activa el modo trueque.
    - Solicita la cantidad de caballos.
    - Valida entrada positiva y entera.
    - Calcula kg de leña equivalentes y los muestra en pantalla.
    - Termina el proceso y devuelve el control al menú.
    """
    global en_trueque, cantidad, kilos, mensaje
    en_trueque = True
    myMenu.close()
    cantidad = game.ask_for_number("Cuántos caballos deseas?", 4)
    ...
    if not (validar_positivo(cantidad)):
        ...
        return
    if not (validar_entero(cantidad)):
        mostrar_error("Error: Solo caballos enteros!")
        en_trueque = False
        reactivar_menu()
        return

    kilos = calcular_lenya_caballos(cantidad)
    mensaje = "" + str(int(cantidad)) + " caballos son " + ("" + str(kilos)) + " kg de leña"
    game.show_long_text(mensaje, DialogLayout.CENTER)
    en_trueque = False
    reactivar_menu()
...
def trueque_huevos():
    """
    Gestiona el trueque de huevos por leña.
    - Cierra menú y activa el modo trueque.
    - Pide al usuario el número de huevos.
    - Valida que sea positivo y entero, sino lanza error.
    - Convierte huevos a kg de leña.
    - Muestra el resultado y vuelve al menú.
    """
    global en_trueque, cantidad, kilos, mensaje
    en_trueque = True
    myMenu.close()
    cantidad = game.ask_for_number("Cuántos huevos deseas?", 4)
    ...
    if not (validar_positivo(cantidad)):
        ...
        return
    if not (validar_entero(cantidad)):
        mostrar_error("Error: Solo huevos enteros!")
        en_trueque = False
        reactivar_menu()
        return

    kilos = calcular_lenya_huevos(cantidad)
    mensaje = "" + str(int(cantidad)) + " huevos son " + ("" + str(kilos)) + " kg de leña"
    game.show_long_text(mensaje, DialogLayout.CENTER)
    en_trueque = False
    reactivar_menu()
...
def menu():
    ...
    global evento_menu_registrado
    paloma.set_flag(SpriteFlag.INVISIBLE, True)
    if evento_menu_registrado:
        return
    ...
    def on_button_pressed(selection, selectedIndex):
        ...
        if selection == "Gallinas":
            trueque_gallina()
        elif selection == "Patatas (kg)":
            trueque_patatas()
        elif selection == "Cabras":
            trueque_cabras()
        elif selection == "Huevos":
            trueque_huevos()
        elif selection == "Caballos":
            trueque_caballos()
        else:
            salir_menu()
    myMenu.on_button_pressed(controller.A, on_button_pressed)
    
    evento_menu_registrado = True

def salir_menu():
    # Cierra el menú y devuelve el control a la paloma.
    global menu_activo
    myMenu.close()
    menu_activo = False
...
def dibujar_menu():
    # Crea el menú visual si no está activo.
    global menu_activo, myMenu, evento_menu_registrado
    if menu_activo:
        return
    menu_activo = True
    # Menú con iconos para cada opción
    myMenu = miniMenu.create_menu(miniMenu.create_menu_item("Gallinas",
            img("""
                . . . . . . . . . b 5 b . . . .
                . . . . . . . . . b 5 b . . . .
                . . . . . . b b b b b b . . . .
                ...
                . . . c c c c c c c c b b . . .
                """)),
        miniMenu.create_menu_item("Patatas (kg)",
            img("""
                . . . . e e e e e e e . . . . .
                . . e e e e e e e e e e e . . .
                . e d e e e e e e e e e e e . .
                . e e e e e e e e e e e e e . .
                e e e e d e e e e e d e e e e .
                e e e e e e e e e e e e e e e .
                e e e e e e e e e e e e e e e .
                e e e e e e e e e 1 e e e e e .
                e e e e e e e e e e e e e e e .
                e e e e e e e e e e e e e e e .
                e e e e d e e e e e d e e e e .
                . e e e e e e e e e e e e e . .
                . e d e e e e e e e e e d e . .
                . . e e e e e e e e e e e . . .
                . . . . e e e e e e e . . . . .
                . . . . . . . . . . . . . . . .
                """)),
        ...
        miniMenu.create_menu_item("Caballos",
            img("""
                . . . e e e e e . . . . . . . .
                . . . f f f f e e e e . . . . .
                . . f 1 f f f f f b b f f . . .
                . f f f f f f f f b b b b f . .
                d f f f f f f f f b b f f f f .
                d d f f . . . f f f f f f f f e
                . . . . . . . f f f f f f f . e
                . . . . . . . f f f . f f f . e
                . . . . . . . f f f . f f f . e
                . . . . . . . f f e . f f e . .
...
def iniciar_juego():
    ...
    # Árbol (nido de la paloma = punto de trueque)
    ...
    # Paloma (el objeto que se mueve = jugador)
...
def on_on_overlap(sprite, otherSprite):
    # Abre el menú al tocar el árbol.
    if menu_activo or en_trueque:
        return
    dibujar_menu()
...
game.splash("Dirige la paloma a su nido.")
iniciar_juego()
